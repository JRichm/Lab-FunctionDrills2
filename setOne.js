...
printParameters('beans', 'joe maima');
...
const greetFullName = (fullName) => console.log(`Hello, my full name is ${fullName}`)
...
greeting('James', 'Richmond', greetFullName);
...
const pricesPlusTax = (pricesList, callback) => {
    let totalCost = [];
    for (let i = 0; i < pricesList.length; i++) {
        totalCost.push(pricesList[i] * 1.20); 
    }
    callback(totalCost);
}

const showTotalCost = (totalCostList) => {
    console.log(`The new array plus tax = ${totalCostList}`);
}
...
// CODE HERE
pricesPlusTax(prices, showTotalCost);
...
const multiplyingFactory = (num1) => {
    return function(num2) {
        if (num1 >= 5) console.log (num1 * num2);
