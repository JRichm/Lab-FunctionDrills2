...
const answer = (product) => console.log(product); 
...
function first(nameList, callback){
  callback(nameList[0]);
}
...
function last(nameList, callback) {
  callback(nameList[nameList.length - 1]);
}
...
function contains(nameList, name, callback) {
...
function uniq(nameList, callback) {
...
// CODE HERE
function removeDupes(uniqArr) {
  console.log(`The new names array with all the duplicate items removed is ${uniqArr}.`)
}

uniq(names, removeDupes);
...
function each(namesList, callback) {
  for (let i = 0; i < namesList.length; i++) {
    callback(namesList[i], i);
  }
}
...
function getUserById(userList, userID, callback) {
  for (let i = 0; i < userList.length; i++) {
    if (userList[i].id === userID) callback(userList[i]);
  }
}
